...
> **生成日期**: 2026-08-03
> **最后更新**: 2026-08-10
> **状态**: takeover、Phase 1 引擎与 Phase 2 CLI 已结束；Phase 3 Web 功能原型进行中，当前重点为真实浏览器试玩、背景图接入与发布验收
...
| `ALARM_MIN` | `360`（分钟）| 06:00 | 工作日闹钟允许的最早设置时间 |
...
引擎内部仍有 `sleeping` 与 `wakeup` 阶段，但当前 Web 将它们自动结算；玩家可见流程为 **4 个 Screen**：

```
    ┌──────────────┐
    │  睡前 Screen  │  ← 1. 以「明早通勤条件」突出显示天气/交通
    │  Bedtime      │    2. 商店购物：先加入可取消的购物车，确认后才扣余额；睡前可重复结算
    │              │    3. 设置闹钟（360~600 / 5min 步进）
    │              │    4. 发当日工资 20 元（Day 1~12 都发）
    └──────┬───────┘
           │ 玩家点击「睡觉」
    ┌──────▼───────┐
    │ Sleep/Wake-up │  引擎自动：计算 SOL、实际入睡时刻、sleepDebt 与 snooze
    │ （无独立 UI） │  当前自动进入醒来后的通勤选择；未来可加入过渡表现
    └──────┬───────┘
           │ 自动进入醒来后的通勤选择
    ┌──────▼───────┐
    │  通勤 Screen  │  先显示当前时刻与距 09:00 的剩余分钟；再以「今日通勤条件」显示天气/交通
    │              │  文字说明昨晚入睡、实际睡眠与闹钟；贪睡次数/时长（不计入睡眠）；25 分钟晨间流程
    │              │  选择地铁/快车/专车后，需点确认按钮才结算
    │              │  → 扣钱 + 取消/故障 roll + 天气/事件加时 + 算到达分钟数
    └──────┬───────┘
           │ 到达公司
    ┌──────▼───────┐
...
```
变量名：sleepDebt（number，单位分钟，始终 ≥ 0；对玩家显示）
隐藏变量：netSleepDebt（number，单位分钟，始终 ≥ 0；不向玩家显示）
...
工作日当晚睡眠结束、计算 snooze 之前：
  ...
  netSleepDebt = netSleepDebt + newDebt  // 不衰减，记录累计缺觉
...
`netSleepDebt` 目前仅供隐藏的长期透支机制与模拟诊断使用，不参与 snooze、坐过站，也不在 UI 或日结日志中显示。

### 4.1.1 试玩调试报告（Telemetry）

引擎会在本地记录每次**有效**操作和结算结果：开局、购买、设闹钟、服用 DORA、睡眠与起床结算、通勤、周末恢复、是否贿赂。到达结局页后，玩家可点击「复制调试 JSON」将报告复制给开发者。

报告包含随机 seed、游戏版本、完整平衡参数、逐日日志、事件序列与结局状态（包括 `netSleepDebt`）；不包含账号、设备、网络或其他个人信息，也不会自动上传。
...
└────┬────┘
     ...
     ▼
┌─────────┐     ┌─────────┐     ┌─────────┐     ┌──────────┐     ┌──────────┐
│ Bedtime │────────────────────▶│ Commute  │────▶│  Office  │
│ Screen  │  引擎自动结算 Sleep/ │  Screen  │     │  Screen  │
│ (商店   │  Wake-up 阶段后进入  │ (3选1    │     │ (打卡/   │
│  +闹钟) │  通勤；当前无独立 UI │  通勤)   │     │  贿赂)   │
└────┬────┘     └─────────┘     └─────────┘     └─────┬────┘     └─────┬────┘
     ▲                                                │                │
     │                                                ▼                │
     │                                            扣通勤费             ▼
...
interface GameState {
  ...
  subwayFailed?: boolean;       // 地铁是否发生故障（CommuteResult.subwayFailed 写回）
  arriveMin?: number;
  isLate?: boolean;
...
### 6.2 渐进教学、规则手册与状态栏（2026-08-07 确认）

- Intro 只介绍目标、睡前准备、通勤和 09:00 deadline，不一次性解释全部规则。
- 睡眠债只在第一次睡醒且实际产生债务后介绍；此前不在开局文案中展开。
- 贿赂机制不在开局说明中出现，只在第一次迟到进入 `bribe` phase 时作为惊喜揭示。
- 永久道具次日到货、DORA、下雪、城市事件和快车取消等机制均在第一次实际遇到时用短教学卡片说明。
- 教学卡片每台设备默认只强制显示一次；规则面板底部提供“重新显示教学提示”，只清除教学已读记录，不清除规则发现记录，也不改变当前游戏进度。
- 主菜单和游戏内均提供同一“规则手册”入口。未发现机制显示为锁定折叠项和剩余数量，不显示具体标题，也不提供提前展开或剧透查看按钮；机制真正触发后才加入完整规则目录。
- 教学已读记录与机制发现记录分别保存在设备本地；不属于游戏数值状态，不写入 reducer 或 `dailyLog`。
- 移动端常驻状态栏显示：当前星期与已完成工作日进度、当前 sleepDebt、当前余额，并提供规则入口。状态栏在 430×932 基线下为紧凑单行；当前时刻、天气/交通、09:00 deadline、背包和通勤详情在对应 Screen 内展示，不挤入常驻数值栏。
- 阶段时间仍由状态推导：Bedtime 为 00:00；Wake-up 与 Commute 都以完成晨间流程后的出门时间为准（当前 UI 直接进入 Commute）；Office/Bribe 为实际抵达时间；不修改引擎时间数据。
- Web 不使用生成式图片。当前原型移除页面内美术占位组件，以纯灰色背景验证信息结构；后续由来源和许可可追溯的 stock 图片经高斯模糊/暗化处理后作为页面背景。状态栏必须完全不透明地遮住背景；内容面板与信息卡可半透明，以便适配背景图但仍保证可读性。

### 6.3 Web 页面纵向布局基线（2026-08-09 确认）

- 以 375×667 CSS px 为小屏下限、430×932 为舒展验证尺寸。状态栏固定为 48px；普通页面在视口内填满可用空间，但内容超过可用高度时必须自然扩展，不能因全局 `overflow: hidden` 被裁切。
- 页面采用「固定信息区 + 弹性留白区 + 固定操作区」的 Flex 布局。背景图只属于背景层，不参与内容高度计算；后续背景比例变化不得改变按钮和关键数值的位置。
- 所有页面共享字体层级：正文/普通操作以 14px 左右为基准，标签与辅助信息约 11–13px，页面标题约 27px 起；同一信息元素内不得混用无意义的字号。

| 页面 | 顶部固定区 | 中部内容/弹性区 | 底部固定区 |
|------|------------|----------------|------------|
| Intro | 游戏标题 | 剧情与目标说明 | 开始、规则入口、seed |
| Bedtime（工作日） | 日期、明早通勤条件（天气/交通） | 单列商店与按需展开的可取消结算条；剩余空间由布局自动吸收 | 紧凑闹钟滚轮、DORA（如有）、睡觉 |
| Bedtime（周末） | 日期、周末说明 | 商店 | 好好休息 |
| Commute | 标题、当前时间、距打卡时间、今日通勤条件（天气/交通）、晨间三段文字说明 | 弹性留白 | 通勤方式、确认按钮 |
| Office / Bribe | 到达/迟到结果 | 结果与原因说明居中 | 继续、贿赂或拒绝 |
| Result | 结果标题、最终余额 | 可局部滚动的每日回顾 | 再来一局 |
...
- 玩家主动选择快车或地铁等含随机风险的方案后因取消/故障失败，仍计入随机性影响统计，但模拟报告必须与“安全参考策略下的不可规避 RNG 失败”分开呈现。
- 因早期消费、闹钟或通勤决策不当造成的资金不足或迟到不归类为纯 RNG 失败。
- 25% 是上限约束，不是期望值；具体实际比例由模拟器数据决定。

### 8.5 D-8 / D-12 模拟验证（2026-08-07～08）
...
- D-12 历史实验在保持 Gradient 60 和债务结转 0.5 不变的前提下，曾加入地铁 5% 故障与 15 分钟加时，并得到固定 24.482%、普通自适应 48.933%、安全参考 97.662% 的结果；该配置目前不是默认配置，数据仅作为历史对照。
...
| P1-2 | 平衡性调整 | 跑模拟器 10 万局后根据通关率微调：SOL / 通勤费 / 坐过站风险 / 取消率 / 下雪概率 | 当前值作为起点；目标口径待 D-10 决策 |
...
| D-12 | 地铁是否继续保持零风险 | **暂缓**：故障配置保留但默认关闭；地铁仍免疫天气/城市事件 | 重新开启需单独模拟和试玩验证 |
