...
- 移动端常驻状态栏显示：当前星期/阶段时间、工作日进度、当前 sleepDebt、当前余额，并提供规则入口。天气/事件、09:00 deadline、背包和通勤详情在对应 Screen 内展示，不挤入常驻数值栏。
- 阶段时间由状态推导：Bedtime 为 00:00；Wake-up 为闹钟加 snooze 时间；Commute 为完成晨间流程后的出门时间；Office/Bribe 为实际抵达时间；不修改引擎时间数据。
- Web 美术不得由生成式图片代替。原型只使用写明位置、功能和建议比例的占位组件；正式资源由美术统一提供像素画，页面支持 `image-rendering: pixelated`。
