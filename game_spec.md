...
固定骨架（保证体验不炸）：
  ...
  Day 4/5: 如果有城市事件叠加，则 30% 概率下雪（偶尔出现地狱局）

随机扰动（增加重玩多样性）：
  其他未指定的工作日（Day 2/3/8/9/10/11）：20% 概率下雪 / 80% 不下雪
```
...
```
事件加时 + 天气加时 = 叠加（无 cap，也不互相抵消）

极端案例（Day 12 Boss 关）：下雪 + 节前高峰
  → 快车/专车：25（基础）+ 15（下雪）+ 20（节前）= 60 分钟
  → 地铁：依然 40 分钟
```
...
| Day 5 | 周五 | #5 | 20% 下雪 / 80% 不下雪；若有事件则再叠 30% 下雪 | **50% 概率** 普通事件（与 Day 4 独立；若都触发则 flavor 不重复）| 第一周收尾：运气不好可能 2 次事件叠加下雪，周末前压力测试。 |
...
function onEnterBedtime(state: GameState): void {
  ...
  // Step 3:  roll 今天的天气 & 事件（仅工作日需要，周末可以不 roll 或 roll 了不用）
