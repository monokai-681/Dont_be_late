...
export const COPY = {
  ...
  web: {
    ...
    rules: {
      ...
      commuteTitle: '通勤', commuteBody: '地铁便宜并免疫天气和交通状况，但有 5% 概率故障并额外耗时 15 分钟；快车便宜但可能取消；专车昂贵但不会取消。',
    ...
    bedtime: {
      workday: '睡前准备', weekend: '周末', conditionsTitle: '明日通勤条件', weekendTitle: '今晚可以放心睡。',
      ...
      resolveCart: '请先确认或取消购物车。', alarm: '闹钟', useDora: (count: number) => `使用 DORA（剩余 ${count}）`, sleep: '睡觉', rest: '好好休息',
    },
    ...
    commute: {
      ...
      sleepSummary: (sleepAt: string, duration: string, alarm: string) => `昨晚 ${sleepAt} 入睡，共睡了 ${duration}；闹钟在 ${alarm} 响起。`,
      snoozeSummary: (count: number, minutes: number) => `你按了 ${count} 次贪睡，共 ${minutes} 分钟。贪睡时间不计入上方睡眠时间。`,
      routineSummary: (minutes: number) => `起床后，穿衣、洗漱、化妆需要 ${minutes} 分钟。`,
      subway: '地铁', express: '快车', premium: '专车',
      subwayRisk: '天气和交通不影响地铁', expressRisk: '小心快车司机取消订单', premiumRisk: '专车不会取消订单',
