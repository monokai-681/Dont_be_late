...
const rejected: Record<ActionRejectedReason, string> = {
  INVALID_ALARM: '请选择 07:00～10:00、5 分钟步长的闹钟。',
...
export const COPY = {
  items: itemNames,
  events: {
    name: (event: EventId, empty = '无城市事件') => event === null ? empty : eventNames[event],
  },
  rejected,
  loseReasons,
  tutorials: {
    sleepDebt: { title: '你欠下了睡债', body: '睡得越少，睡债越高，第二天越可能多按几次 snooze。旧债每天会恢复一半。' },
  ...
  } satisfies Record<MechanicId, { title: string; body: string }>,
  shop: {
  ...
  web: {
    ...
    status: { time: '时间', progress: '进度', debt: '睡债', balance: '余额', rules: '规则', rulesAria: '打开规则手册' },
    tutorialCard: { eyebrow: '新机制', dismiss: '知道了' },
    rules: {
      title: '规则手册', close: '关闭', goalTitle: '打卡目标',
      goalBody: '在 Day 1～12 中完成 10 个工作日打卡。超过 09:00 到达算迟到。',
      nightTitle: '每天晚上', nightBody: '查看预报、购买道具并设置 07:00～10:00 的闹钟。余额不能为负。',
      commuteTitle: '通勤', commuteBody: '地铁便宜并免疫天气和城市事件，但有 5% 概率故障并额外耗时 15 分钟；快车便宜但可能取消；专车昂贵但不会取消。',
    ...
    intro: { eyebrow: '不能再迟到了！', title: '别迟到', body: '你是公司的迟到大王：本年度你已经迟到 19 次。再迟一次就要被开除了！本年还剩 10 个工作日，保证 8 小时的睡眠。早晨选通勤方式，在 09:00 前赶到公司。避免再次迟到。', start: '开始第一晚', rules: '规则手册', seed: (seed: string) => `本局 seed：${seed}` },
    ...
    bedtime: { workday: '睡前准备', weekend: '周末', workdayTitle: '明早别迟到。', weekendTitle: '今晚可以放心睡。', snow: '❄ 下雪', clear: '○ 无雪', shop: '商店', owned: '已拥有', pending: '配送中', alarm: '闹钟', useDora: (count: number) => `使用 DORA（剩余 ${count}）`, sleep: '睡觉', rest: '好好休息' },
    ...
    commute: { leave: '出门', title: '选择通勤方式', deadline: '打卡截止', subway: '地铁', express: '快车', premium: '专车', subwayRisk: '天气和城市事件不影响地铁', expressRisk: '小心快车司机取消订单', premiumRisk: '专车不会取消订单' },
    office: { success: '打卡成功', onTime: (time: string) => `${time} · 准时`, expressCancelled: '快车被取消一次，重新叫车后到达。', subwayFailed: '地铁发生信号故障，额外耽误了 15 分钟。', end: (balance: number) => `今日结束，余额 ¥${balance}`, next: '进入下一天' },
    bribe: { late: (time: string) => `迟到 · ${time}`, title: '主管把你叫到一边。', expressCancelled: '快车取消过一次，重新叫车后仍然迟到了。', subwayFailed: '地铁发生信号故障，额外耽误了 15 分钟。', body: (cost: number) => `你懂什么意思，用 ¥${cost} 贿赂主管，这次迟到可以不计入记录，下不为例。`, pay: (cost: number) => `支付 ¥${cost}`, decline: '拒绝' },
  },
  cli: {
    ...
    eventEmpty: '无', clear: '无雪', snow: '下雪',
    shopPrompt: '购买：', invalidShop: '请输入 0～5。', alarmPrompt: '设置闹钟（07:00～10:00，5分钟步长）：',
