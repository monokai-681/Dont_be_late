...
export function calculateSOL(
  ...
  const base = config.SOL_BASE_OVERRIDE ?? SOL_BASE;
  let sol = base;

  // 可重复永久道具：次日晚到货的已在 inventory 里置 true
  if (inventory.pillow)   sol += config.SOL_PILLOW_REDUCTION;
  if (inventory.eyeMask)  sol += config.SOL_EYE_MASK_REDUCTION;
  if (inventory.earPlugs) sol += config.SOL_EAR_PLUGS_REDUCTION;

  // 消耗品 DORA（当晚买了立刻能用，无耐药，-15）
  if (doraUsedTonight) sol += config.SOL_DORA_REDUCTION;
