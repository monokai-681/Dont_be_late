...
export interface BalanceConfig {
  ...
  readonly COMMUTE_SUBWAY_MISSED_STOP_DEBT_THRESHOLD: number;
  readonly COMMUTE_SUBWAY_MISSED_STOP_DEBT_CAP: number;
  readonly COMMUTE_SUBWAY_MISSED_STOP_EXTRA_MIN: number;
...
export const DEFAULT_BALANCE_CONFIG: Readonly<BalanceConfig> = Object.freeze({
  ...
  // 暂停中的彩蛋机制：保留代码和可注入配置，但默认不激活，也不消耗 RNG roll。
  COMMUTE_SUBWAY_FAILURE_RATE: 0,
  COMMUTE_SUBWAY_FAILURE_EXTRA_MIN: 15,
  // 连续亏觉的额外风险：180 分钟起线性增加，300 分钟及以上必定坐过站。
  COMMUTE_SUBWAY_MISSED_STOP_DEBT_THRESHOLD: 180,
  COMMUTE_SUBWAY_MISSED_STOP_DEBT_CAP: 300,
  COMMUTE_SUBWAY_MISSED_STOP_EXTRA_MIN: 20,
