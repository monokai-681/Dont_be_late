...
- [x] 通勤 3 档（移除「开车」）：🚇 地铁免疫天气/城市事件；故障机制暂时关闭；睡债 180～300 时坐过站概率由 0% 线性升至 100%、+20min / 🚕 快车 30% 取消 / 🚘 专车 0% 取消但不免疫天气事件
...
- [x] 原型移除项确认：开车、限行、车牌、油电区分、唑吡坦（传统安眠药）、纯随机堵车概率仍移除；地铁信号故障代码保留但默认关闭；高睡债坐过站风险当前生效
...
- [x] D-13 当前值：地铁故障机制默认关闭；睡债 180～300 时坐过站概率由 0% 线性升至 100%、+20min；坐过站优先于故障 roll
...
- [x] `calculateCommute()`：地铁为 60m/5¥、免疫天气/事件；睡债 180～300 时坐过站概率由 0% 线性升至 100%、+20min；坐过站优先于暂时关闭的故障机制；回归测试覆盖天气/事件组合、风险阈值及不可变默认配置
...
D-12 历史 100,000 局（同一 base seed）曾加入地铁 5% 故障 +15min：固定 24.482%、普通 48.933%、安全 97.662%；安全策略纯 RNG 失败 2.338%，仍通过 D-9。当前默认关闭故障，该数据仅作历史对照。

D-13 历史 100,000 局（同一 base seed）曾使用睡债≥200、30% 坐过站 +20min；当前代码已改为 180～300 的线性概率曲线，需用当前配置重新完成正式模拟。该风险使无购物的 07:00 地铁路线无法稳定速通。
...
- [x] 页面内 CSS 美术占位已移除；当前使用灰色背景验证布局，后续接入来源和许可可追溯、经高斯模糊的 stock 背景图（不使用生成式图片）
...
- [~] P1-2 平衡性调整：当前高睡债坐过站机制已落实；仍待用当前配置完成 10 万局正式模拟，并结合真实试玩校准 SOL / 通勤费 / 坐过站风险 / 取消率 / 下雪概率
...
- [~] D-10 通关率口径：当前线性坐过站曲线待完成正式模拟；暂不追求特定纯 RNG 失败率，留待真实试玩校准
- [x] D-11 余额定位：通关为主目标，最终余额为通关后的次级分数；失败局余额只展示、不参与成绩比较
- [~] D-12 地铁故障：故障率配置和 +15 分钟逻辑保留，但默认关闭；重新开启前需重新模拟和试玩
- [x] D-13 高睡债地铁风险：sleepDebt 180～300 时坐过站概率由 0% 线性升至 100%，额外增加 20 分钟；坐过站优先；已落实代码、UI、CLI、测试与模拟器
...
- [ ] P2-6 预留：后续版本加回开车 / 限行 / 事故；地铁故障逻辑已保留但默认关闭
...
- 通勤三档：🚇 地铁 60m/5元(免疫天气/事件；睡债180～300 时坐过站0%→100%、+20m；故障机制暂停) | 🚕 快车 25m/30元(30%取消+10m) | 🚘 专车 25m/60元(不取消)
