...
- [~] D-13 当前值：地铁故障机制暂时关闭；睡债 180～300 时坐过站概率由 0% 线性升至 100%、+20min；当前参数待重新完成正式模拟
...
D-13 正式 100,000 局（同一 base seed）新增睡债≥200 时、未故障地铁 30% 坐过站 +20min：固定 12.714%、普通 48.933%、安全 80.402%；安全策略纯 RNG 失败 19.598%，仍通过 D-9。该风险使无购物的 07:00 地铁路线无法稳定速通。
...
- [~] P1-2 平衡性调整：D-13 已完成高睡债地铁坐过站机制；当前线性曲线待完成 10 万局验证，SOL / 通勤费 / 取消率 / 下雪概率是否继续调整，待 D-10 目标口径确认
