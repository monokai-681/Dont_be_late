...
| 1 | 无 UI 核心引擎 | ✅ 完成（1-1~1-5✅） | 完整 reducer、商店、三策略模拟器和 127 项测试已交付 |
...
- [x] 回归测试覆盖统计守恒、可复现性、策略筛选、报告格式与非法参数；全项目共 127 项测试
...
- [ ] D-10 通关率口径：已有第二轮 10 万局数据——固定 32.051%、普通 60.786%、安全 99.994%；需决定主验收策略和正式目标区间
...
1. 决定 D-10 的主验收策略与正式目标通关率区间
2. 如 D-10 要求继续调参，使用可注入 `BalanceConfig` 做定向扫描
3. 进入 Phase 2 CLI
