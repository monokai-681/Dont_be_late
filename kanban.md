...
| 1 | 无 UI 核心引擎 | ✅ 完成（1-1~1-5✅） | 完整 reducer、商店、三策略模拟器和 120 项测试已交付 |
...
- [x] 回归测试覆盖统计守恒、可复现性、策略筛选、报告格式与非法参数；全项目共 120 项测试
...
- [!] D-8 固定保守策略：首轮实验组已实现但模拟验证失败——默认 seed 10,000 局中 07:00＋地铁 100% 通关；需在更早 deadline、提高 snooze 上限、去掉债务衰减或组合调整中作二次决策
- [x] D-9 RNG 公平性：允许不可规避的纯 RNG 失败，但安全参考策略下整局纯 RNG 失败率必须 <25%；具体统计口径见 game_spec §8.4
- [ ] D-10 通关率口径：已有首轮分策略数据，但当前固定与安全策略均 100%；需在 D-8 二次方案后决定主验收策略和正式目标区间
...
1. 用户确认 D-8 二次平衡方案
2. 用可注入 `BalanceConfig` 做候选参数扫描，比较固定/普通自适应/安全参考策略
3. 决定 D-10 的主验收策略与正式目标通关率区间
4. 运行 10 万局正式验证后进入 Phase 2 CLI
