...
- [x] D-8 第二轮正式值：09:00 打卡；地铁 60min/5元且风险 0%；snooze 每次 9min、上限 6 次；工作日/周末每日债务结转 0.5；`SNOOZE_GRADIENT=60`；10 万局结果为固定 32.051%、普通 60.786%、安全 99.994%
...
- [x] Intro / Bedtime / Sleep / Wake-up / Commute / Office+Bribe / Result 七个逻辑页面
...
- [x] 状态栏：星期/时间、工作日进度、睡眠债、余额、规则入口
- [x] 所有场景均为标明位置、功能和比例的 CSS 美术占位，不生成图片
...
- [ ] 根据试玩修正状态栏密度、商店操作和规则面板阅读体验

#### 3-4 视觉与交付 ⏳

- [ ] 与美术确认像素画资源清单、比例、状态变体和替换命名
- [ ] 替换占位资源后做最终响应式与可访问性检查
...
- [~] P1-2 平衡性调整：D-8 第二轮已完成 10 万局验证；SOL / 通勤费 / 取消率 / 下雪概率是否继续调整，待 D-10 目标口径确认
...
- [~] D-10 通关率口径：已有第二轮 10 万局数据——固定 32.051%、普通 60.786%、安全 99.994%；暂不追求特定纯 RNG 失败率，留待真实试玩校准
